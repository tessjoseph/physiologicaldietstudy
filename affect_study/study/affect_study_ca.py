    ...
    def __create_prompt(self, i, format_instructions):
        if i == 0:
            return PromptTemplate(template=(
                ("As a Chinese American undergraduate college student \
                who was born and raised in either the United States or \
                a Chinese context (including China, Hong Kong, Taiwan, Indonesia, Malaysia, \
                and Singapore, is fluent in English, more oriented to American \
                than Chinese culture but still moderately oriented to Chinese culture, \
                and has parents and grandparents of Chinese descent who were born and raised in  a \
...
logging.basicConfig(filename=fname + "_us_affect_study_ca_final.log", encoding='utf-8', level=logging.DEBUG)
for i in range(1):  # choose n for how many times to run this survey
    df = analyzer.generate_response()
    df.to_csv("results/final/" + fname + "/us_affect_study_final_ca" + fname + str(63+i))
