...
#todays tasks
#generate enough data to analyze -> use the mixed emotions study as a reference or mimic the real study with the LLM study
#get the data and analyze it in jupyter notebook
...
class AffectStudy:
    ...
    This is affect_us.py: replicating the sample to assess LLM's understanding of European American individuals' emotions
    ...
    def __init__(self, model_name):
        ...
        if model_name == ("meta-llama/llama-3.1-70b-instruct"):
            self.name = "llama-3.1-70b-instruct"
        ...
        self.feelings = {
            ...
            "strong (elated)": "Feeling physically or emotionally powerful, combined with a heightened\
            ...
            "lonely": "Feeling isolated or disconnected from others, often leading to sadness or longing \
    ...
    def __create_model(self, name):
        model = ChatOpenAI(
            model=name,
            # my api key -> openai_api_key="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",
        ...
        for feeling, feeling_description in self.feelings.items():
            ...
            for j in range(1):
                if j == 0:
                    ...
                    i = 0
                    attempts = 0
                    max_attempts = 5  # Set a maximum number of attempts per feeling
    ...
    def __create_prompt(self, i, format_instructions):
        if i == 0:
            return PromptTemplate(template=(
                "As a European American undergraduate college student who is fluent in English,\
                highly oriented to American culture, and has parents of European descent, \
                please provide a comprehensive rating of how often you would like to feel \
                and how often you actually feel each feeling. \
                Some of the feelings are very similar to each other, whereas others are very \
                different from each other. Read each {feeling} and each feeling description \
                {feeling_description} and thenRATE how often YOU IDEALLY WOULD LIKE TO HAVE \
                each {feeling} FEELING and how often YOU ACTUALLY HAVE each {feeling} FEELING \
                over the course of a typical week, using the following 5-point scale \
                with labels at 1 (never) to 5 (all the time). \
...
# choose an LLM to explore by uncommenting the line with name=(<LLM name>)
...
logging.basicConfig(filename=fname + "_us_affect_study_us_final.log", encoding='utf-8', level=logging.DEBUG)
for i in range(39):  # choose n for how many times to run this survey
    df = analyzer.generate_response()
    df.to_csv("results/final/" + fname + "/us_affect_study_final_us" + fname + str(25+i))
